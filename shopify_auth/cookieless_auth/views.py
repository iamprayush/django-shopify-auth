...
def get_scope_permission(request, myshopify_domain):
    redirect_uri = request.build_absolute_uri(reverse("cookieless_auth:finalize"))
...
class FinalizeAuthView(View):
    def get(self, request):
        ...
        shop, created = get_user_model().objects.update_or_create(myshopify_domain=shopify_session.url, defaults={"token": shopify_session.token}) 

        shop.install(request)

        return HttpResponseRedirect(get_return_address(request) + f"?shop={myshopify_domain}")
